...
function buildInitialGarage() {
  console.log('🏗️ buildInitialGarage: iniciando criação da garagem...');
  console.log('🏗️ FLOORS:', FLOORS);
  console.log('🏗️ SIDES_BY_FLOOR:', SIDES_BY_FLOOR);
  console.log('🏗️ NATURAL_PAIRS:', NATURAL_PAIRS);
  ...
  for (const floor of FLOORS) {
    for (const side of SIDES_BY_FLOOR[floor]) {
      for (const [p1, p2] of NATURAL_PAIRS) {
        ...
        console.log(`🏗️ Criando par: ${parId} -> A:${aId}, B:${bId}`);
  ...
  console.log('🏗️ buildInitialGarage: criados', Object.keys(pairs).length, 'pares');
  console.log('🏗️ buildInitialGarage: criados', spots.length, 'spots');
  console.log('🏗️ buildInitialGarage: pares criados:', Object.keys(pairs));
...
export default function GarageLotteryApp() {
  ...
  const pick = (arr) => {
    ...
    if (!result && arr.length === 0) {
      console.log(`⚠️ [pick] Array vazio recebido!`);
    }
    return result;
  };

  const getFreePairs = (state) => {
    console.log('🔍 ========== ANÁLISE DETALHADA DE getFreePairs ==========');
    console.log('🔍 Vagas estendidas esperadas: [7, 8, 21, 22, 35, 36]');

    // Debug das configurações
    console.log('🔍 VAGAS_CONFIG.VAGAS_POR_LADO:', VAGAS_CONFIG.VAGAS_POR_LADO);
    console.log('🔍 NATURAL_PAIRS:', NATURAL_PAIRS);
    console.log('🔍 FLOORS:', FLOORS);
    console.log('🔍 SIDES_BY_FLOOR:', SIDES_BY_FLOOR);
    console.log('🔍 Total de pares disponíveis:', Object.keys(state.pairs).length);
    ...
    for (const id in state.pairs) {
      ...
      if (!a || !b) {
        console.log(`❌ Par ${p.id}: vaga A ou B não encontrada (A:${!!a}, B:${!!b})`);
        continue;
      }

      // Converte IDs das vagas para números sequenciais
      ...
      console.log(`🔍 Par ${totalPares}: ${p.id} → A:${a.floor}-${a.side}${a.pos}(seq=${vagaNumA}) + B:${b.floor}-${b.side}${b.pos}(seq=${vagaNumB})`);

      // Verifica se são estendidas
      const aEstendida = isVagaEstendida(vagaNumA);
      const bEstendida = isVagaEstendida(vagaNumB);

      console.log(`🔍   → Estendidas? A(${vagaNumA})=${aEstendida}, B(${vagaNumB})=${bEstendida}`);

      if (aEstendida || bEstendida) {
        paresExcluidos++;
        console.log(`🚫   → Par ${p.id} EXCLUÍDO por ter vaga estendida`);
        continue;
      }

      // Verifica se estão livres
      const aLivre = !a.occupiedBy && !a.blocked;
      const bLivre = !b.occupiedBy && !b.blocked;

      console.log(`🔍   → Livres? A=${aLivre}(ocu:${a.occupiedBy},bloc:${a.blocked}), B=${bLivre}(ocu:${b.occupiedBy},bloc:${b.blocked})`);

      if (aLivre && bLivre) {
        console.log(`✅   → Par ${p.id} ADICIONADO como LIVRE!`);
        free.push(p);
      } else {
        paresOcupados++;
        console.log(`❌   → Par ${p.id} ocupado/bloqueado`);
      }
    }

    console.log('🔍 ========== RESUMO FINAL ==========');
    console.log('🔍 Total de pares:', totalPares);
    console.log('🔍 Pares excluídos (contém vaga estendida):', paresExcluidos);
    console.log('🔍 Pares ocupados/bloqueados:', paresOcupados);
    console.log('🔍 Pares livres:', free.length);
    console.log('🔍 Pares livres IDs:', free.map(p => p.id));
    console.log('🔍 ==========================================');
  ...
  const chooseBalancedSpot = (spotsList, state) => {
    console.log(`🎯 [chooseBalancedSpot] Lista recebida:`, spotsList?.length || 0, 'spots');
    console.log(`🎯 [chooseBalancedSpot] Spots IDs:`, spotsList?.map(s => s.id) || []);

    if (!spotsList || spotsList.length === 0) {
      console.log(`❌ [chooseBalancedSpot] Lista vazia ou null!`);
      return null;
    }

    if (spotsList.length <= 1) {
      const chosen = pick(spotsList);
      console.log(`✅ [chooseBalancedSpot] Escolhido (lista pequena):`, chosen?.id || 'null');
      return chosen;
    }
    ...
    console.log(`✅ [chooseBalancedSpot] Escolhido (balanceado):`, chosen?.id || 'null');
    return chosen;
  };
  ...
  */
  const runPreprocessIfNeeded = () => {
    console.log('🏁 runPreprocessIfNeeded: INICIANDO');
    console.log('🏁 preprocessed:', preprocessed, 'doubleReservations:', !!doubleReservations, 'extendedReservations:', !!extendedReservations);

    if (preprocessed && doubleReservations && extendedReservations) {
      console.log('🏁 Pré-processo já realizado, pulando...');
      return;
    }

    console.log('🏁 Executando pré-processo...');
    const state = structuredClone({ spots: garage.spots, pairs: garage.pairs });

    // === RESERVA DE PARES PARA DUPLAS ===
    console.log('🏁 FASE 1: Reservando pares para apartamentos duplos');

    const duplos = apartments.filter((a) => a.dupla && a.ativo).map((a) => a.id);
    console.log('🏁 Apartamentos duplos encontrados:', duplos.length, '→', duplos);

    const freePairs = getFreePairs(state);
    console.log('🏁 Pares livres encontrados:', freePairs.length, '→', freePairs.map(p => p.id));

    if (duplos.length > freePairs.length) {
      console.error('🚨 PROBLEMA ENCONTRADO!');
      console.error('🚨 Apartamentos duplos:', duplos.length);
      console.error('🚨 Pares livres:', freePairs.length);
      console.error('🚨 Diferença:', duplos.length - freePairs.length, 'pares em falta');
      alert(
    ...
    for (const aptId of orderDuplas) {
      ...
      log("Pré-processo: reservado par", chosen.id, "para apartamento duplo", aptId);
    }
    ...
    if (extendidos.length > freeExtended.length) {
      alert(
        `Não há vagas estendidas suficientes: apartamentos=${extendidos.length}, vagas estendidas livres=${freeExtended.length}`
      );
      return;
    }
    ...
    for (const aptId of orderEstendidas) {
      ...
      log("Pré-processo: reservada vaga estendida", vagaNum, "para apartamento", aptId);
    }
  ...
  const drawOne = () => {
    ...
    if (apt.dupla) {
      ...
      setGarage((prev) => {
        ...
        console.log(`🎯 [drawOne] Par selecionado para apt ${apt.id}:`, pair);
        if (!pair) {
      ...
      setGarage((prev) => {
        ...
        if (apt.estendida && extendedReservations?.[apt.id]) {
          ...
          // Verifica se a vaga reservada ainda está livre
          if (reservedSpot && !reservedSpot.occupiedBy) {
            ...
            console.log(`🎯 Apartamento extendido ${apt.id} recebeu vaga estendida reservada ${vagaNum}`);
          }
        }


        // Se não conseguiu usar a reserva, busca vaga disponível
        if (!chosenSpot) {
          // Lista de vagas livres (sem reservas de pares e não ocupadas)
          const allFree = prev.spots.filter(
            (s) =>
              !s.blocked &&
              !s.occupiedBy &&
              !prev.pairs[s.parId]?.reservedFor &&
              !s.reservedFor // não está reservada para outro apartamento extendido
          );

          console.log(`🔍 [drawOne] Apt ${apt.id} - Vagas totalmente livres:`, allFree.length);
          console.log(`🔍 [drawOne] Apt ${apt.id} - IDs das vagas livres:`, allFree.map(s => s.id));
          console.log(`🔍 [drawOne] Apt ${apt.id} - Detalhes vagas livres:`, allFree.map(s => ({
            id: s.id,
            floor: s.floor,
            side: s.side,
            pos: s.pos,
          ...
          if (!allFree.length) {
            console.log(`❌ [drawOne] Apt ${apt.id} - SEM VAGAS LIVRES! Mostrando análise detalhada:`);
            console.log(`🔍 Total spots:`, prev.spots.length);
            console.log(`🔍 Spots bloqueados:`, prev.spots.filter(s => s.blocked).length);
            console.log(`🔍 Spots ocupados:`, prev.spots.filter(s => s.occupiedBy).length);
            console.log(`🔍 Spots com par reservado:`, prev.spots.filter(s => prev.pairs[s.parId]?.reservedFor).length);
            console.log(`🔍 Spots reservados para apt extendido:`, prev.spots.filter(s => s.reservedFor).length);
            alert("Sem vaga disponível.");
            return prev;
          }

          // Separa vagas estendidas e vagas normais disponíveis
          ...
          if (apt.estendida) {
            ...
            if (extendedFree.length > 0) {
              ...
              const vagaNum = positionToSequentialNumber(chosenSpot.floor, chosenSpot.side, chosenSpot.pos);
              console.log(`🟠 Apartamento extendido ${apt.id} recebeu vaga estendida ${vagaNum}`);
            } else if (normalFree.length > 0) {
              // fallback: se não sobrou nenhuma estendida livre, pode usar vaga normal
              chosenSpot = chooseBalancedSpot(normalFree, prev);
              const vagaNum = positionToSequentialNumber(chosenSpot.floor, chosenSpot.side, chosenSpot.pos);
              console.log(`✅ Apartamento extendido ${apt.id} recebeu vaga normal ${vagaNum} (sem vagas estendidas disponíveis)`);
            }
          } else {
            // Apartamento simples:
            console.log(`🔍 [drawOne] Apt ${apt.id} é SIMPLES - analisando vagas disponíveis...`);
            // 1) Enquanto houver vaga normal, só pode receber vaga normal
            // 2) Se acabarem as vagas normais, pode usar vaga estendida que sobrou
            if (normalFree.length > 0) {
              chosenSpot = chooseBalancedSpot(normalFree, prev);
              const vagaNum = positionToSequentialNumber(chosenSpot.floor, chosenSpot.side, chosenSpot.pos);
              console.log(`✅ Apartamento simples ${apt.id} recebeu vaga normal ${vagaNum}`);
            } else if (extendedFree.length > 0) {
              chosenSpot = chooseBalancedSpot(extendedFree, prev);
              const vagaNum = positionToSequentialNumber(chosenSpot.floor, chosenSpot.side, chosenSpot.pos);
              console.log(`🟡 Apartamento simples ${apt.id} recebeu vaga estendida ${vagaNum} (todas as vagas normais ocupadas)`);
            } else {
              console.log(`❌ [drawOne] Apt ${apt.id} SIMPLES - SEM VAGAS! normalFree=${normalFree.length}, extendedFree=${extendedFree.length}`);
              alert(`Apartamento simples ${apt.id} não pode receber vaga - todas as vagas estão ocupadas.`);
              return prev;
            }
          }
        }
        ...
        if (!chosenSpot) {
          ...
          console.log(`🚨 [drawOne] Apt ${apt.id} - NÃO SERÁ MARCADO COMO SORTEADO (sem vaga disponível)`);
          alert(`Sem vaga disponível para apartamento ${apt.id}.`);
          return prev;
        }

        console.log(`✅ [drawOne] Apt ${apt.id} - Vaga escolhida:`, chosenSpot.id);

        // 🔒 PROTEÇÃO: Só marca como sorteado APÓS confirmar que tem vaga válida
        if (!chosenSpot.id) {
          console.log(`❌ [drawOne] Apt ${apt.id} - Vaga escolhida não tem ID válido!`);
          alert(`Erro interno: vaga inválida para apartamento ${apt.id}.`);
          return prev;
        }
        ...
        // ✅ CORREÇÃO TOTAL: Estados síncronos para evitar condição de corrida
        console.log(`✅ [drawOne] Apt ${apt.id} - AGORA marcando como sorteado e atribuindo vaga ${chosenSpot.id}`);
        console.log(`🔒 [drawOne] Apt ${apt.id} - Proteção: vaga confirmada antes de marcar sorteado`);

        // 🔄 ESTADO SÍNCRONO: Atualizar apartments primeiro, garage depois
        setApartments((prevApts) => {
          const updatedApts = prevApts.map((a) =>
            a.id === apt.id ? { ...a, sorteado: true, vagas: [chosenSpot.id] } : a
          );

          console.log(`🔄 [drawOne] Apt ${apt.id} - Estado apartments atualizado ANTES de garage`);

          // 🔄 Depois atualizar garage (garantindo sincronização)
          setTimeout(() => {
  ...
  /* 🔧 CORREÇÃO DE ÓRFÃOS - SOLUÇÃO ATÔMICA */
  const fixOrphanedApartments = () => {
    console.log('🔧 [CORREÇÃO ATÔMICA] Iniciando...');

    // ⚡ CAPTURA DE ESTADO ATUAL (sem callbacks)
    ...
    console.log(`🔧 Órfãos encontrados: ${orphanedApts.length}`, orphanedApts.map(a => a.id));
    console.log(`🔧 Vagas livres: ${freeSpots.length}`, freeSpots.map(s => s.id));

    if (orphanedApts.length === 0) {
      alert('✅ Nenhum apartamento órfão encontrado!');
      return;
    }

    // 📋 CALCULAR TODAS AS CORREÇÕES
    const fixes = [];
    ...
    console.log('� Correções planejadas:', fixes);

    // ⚡ APLICAÇÃO ATÔMICA - APARTMENTS
    const newApts = apartments.map(apt => {
      const fix = fixes.find(f => f.aptId === apt.id);
      if (fix) {
        console.log(`✅ ${apt.id} → Vaga ${fix.vagaId}`);
        return { ...apt, vagas: [fix.vagaId] };
      }
      return apt;
    });

    // ⚡ APLICAÇÃO ATÔMICA - GARAGE  
    ...
    alert(`✅ ${fixes.length} apartamentos órfãos corrigidos!`);
    console.log(`🎯 Correção concluída: ${fixes.length} correções aplicadas`);
  };
  console.log('✅ [fixOrphanedApartments] Nenhum órfão encontrado!');
  return prevApts;
}

// � SINCRONIZAÇÃO: Atualiza garage e apartments em sequência controlada
setGarage((prevGarage) => {
  const freeSpots = prevGarage.spots.filter(s => !s.blocked && !s.occupiedBy && !s.reservedFor);

  console.log(`🔧 [fixOrphanedApartments] Vagas livres encontradas: ${freeSpots.length}`);
  console.log(`🔧 [fixOrphanedApartments] IDs vagas livres:`, freeSpots.map(s => s.id));
  ...
  orphanedApts.forEach((orphan, index) => {
    if (index < freeSpots.length) {
      ...
      if (spotIndex >= 0) {
        ...
        fixedCount++;
        console.log(`✅ [fixOrphanedApartments] ${orphan.id} → Vaga ${chosenSpot.id}`);
      }
    }
  });

  console.log(`🎯 [fixOrphanedApartments] ${fixedCount} vagas alocadas`);
  return { ...prevGarage, spots: updatedSpots };
});

// 🔄 Atualiza apartamentos com as vagas
  ...
  orphanedApts.forEach((orphan, index) => {
    ...
    if (index < freeSpots.length) {
      ...
      if (aptIndex >= 0) {
        ...
        console.log(`✅ [fixOrphanedApartments] Apt ${orphan.id} recebeu vaga ${chosenSpot.id}`);
      }
    }
  });

  console.log(`🎯 [fixOrphanedApartments] Correção concluída!`);
  return updatedApts;
});
};
    ...
    <div className={`wrap ${compact ? "compact" : ""}`}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
        ...
        <button
          ...
          title="Corrige apartamentos marcados como sorteados mas sem vaga atribuída"
        >
          🔧 Corrigir Órfãos
        </button>
          ...
          >
            ...
            {apartments.length === 0 && (
              <div style={{ color: '#ef4444', padding: 16, background: '#fef2f2', borderRadius: 8, marginBottom: 16 }}>
                ⚠️ Nenhum apartamento carregado. Verificando configuração...
              </div>
            )}
            <div
              style={{
