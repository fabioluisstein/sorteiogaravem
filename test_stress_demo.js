/**
 * @fileoverview Script de Teste de Stress - 10 Sorteios para Demonstração
 * @description Executa 10 sorteios usando o código real do projeto para verificar se tudo funciona
 */
...
function validarSorteioCompleto(resultados) {
    ...
    resultados.forEach((resultado, index) => {
        ...
        if (isDuplo) {
            ...
            const vagaId = vagas[0];
            if (sorteioConfig.vagasEstendidas.includes(vagaId)) {
                erros.push(`Sorteio ${index + 1}: ⚠️  CRITICO: Apartamento simples ${apartment.id} recebeu vaga estendida ${vagaId}`);
            }
        }
    });
